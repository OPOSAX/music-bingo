...
module.exports = class Room {
    constructor(room_id, worker, io) {
        ...
        this.peers = new Map();
        this.bannedPeers = [];
        this.chatHistory = []; // Persiste mensajes del chat durante la sesión
    ...
    addChatMessage(data) {
        ...
        // Limitar historial a 500 mensajes para evitar crecimiento ilimitado
    ...
    // Concert Mode: options { appData, paused, announce }. Con announce:false el producer no se
    // anuncia a los demás peers (solo lo consumen dj/admin/audio-engine).
    async produce(socket_id, producerTransportId, rtpParameters, kind, type, options = {}) {
        if (!this.peers.has(socket_id)) return;

        const peer = this.getPeer(socket_id);

        const peerProducer = await peer.createProducer(producerTransportId, rtpParameters, kind, type, options);
        ...
        if (options.announce === false) return id;
