...
usa **la misma sala que la partida**: el jugador escanea el QR de acceso, abre su tarjeta y junto al botón de la letra
encuentra **🎤 Quiero cantar**; el anfitrión ve la lista "Quieren cantar" bajo los controles de la canción (`#/host`)
y en el panel de transmisión (`#/live`). Con **Autorizar** el servidor prepara el micrófono del teléfono y lo pone en
vivo en cuanto está listo (el audio suena por el equipo del anfitrión); Silenciar y Terminar siempre a mano. El
"Panel del DJ" (`#/dj`) queda para el control avanzado: mezclador, cancelación de eco, varios micrófonos. `#/sing`
sigue existiendo para el QR independiente del DJ (karaoke sin bingo):
